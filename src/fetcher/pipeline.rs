...
use crate::fetcher::slot_time::date_to_unix_ts;
use crate::fetcher::slot_time::estimate_slot_for_timestamp;
...
impl FetchPipeline {
    pub fn new(config: Config) -> Result<Self> {
        ...
        let cache  = Arc::new(TxCache::default_dir()?);
...
async fn run_async(&self, slot_start: u64, slot_end: u64) -> Result<FetchResult> {
    tracing::info!(
        "FetchPipeline starting: wallet={} slots=[{}-{}]",
        self.wallet.as_str(),
        slot_start,
        slot_end
    );
    ...
    tracing::info!(
        "Inventory loaded: {} positions across {} pools",
    ...
    tracing::info!(
        "Fetch complete: {} transactions across {} pools, cached to .lvr-cache/",
    ...
    /// Convenience constructor that converts dates to slots automatically
    pub fn run_for_dates(
        &self,
        from: chrono::NaiveDate,
        to:   chrono::NaiveDate,
    ) -> Result<FetchResult> {
        let slot_start = estimate_slot_for_timestamp(
            date_to_unix_ts(from),
            &self.client,
        )?;
        let slot_end = estimate_slot_for_timestamp(
            date_to_unix_ts(to),
            &self.client,
        )?;
...
mod tests {
    ...
    #[test]
    fn fetch_result_total_transactions() {
